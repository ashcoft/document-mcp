...
class TestSettingsDefaults:
    ...
    def test_database_url_default(self):
        ...
        settings = Settings()
        assert settings.db_url is not None  # noqa: S101
        assert "postgresql" in settings.db_url  # noqa: S101

    def test_ollama_host_default(self):
        """Verify default Ollama host is localhost."""
        settings = Settings()
        assert settings.ollama_host == "http://localhost:11434"  # noqa: S101

    def test_ollama_model_default(self):
        """Verify default Ollama model is set."""
        settings = Settings()
        assert settings.ollama_model == "llama3.1:8b-instruct-q4_K_M"  # noqa: S101

    def test_embed_model_default(self):
        """Verify default embedding model is set."""
        settings = Settings()
        assert settings.embed_model == "BAAI/bge-small-en-v1.5"  # noqa: S101

    def test_web_ui_defaults(self):
        """Verify default web UI settings."""
        settings = Settings()
        assert settings.start_web_ui is True  # noqa: S101
        assert settings.web_port == 8000  # noqa: S101
        assert settings.web_host == "0.0.0.0"  # noqa: S101

    def test_ocr_defaults(self):
        """Verify default OCR settings."""
        settings = Settings()
        assert settings.ocr_dpi == 300  # noqa: S101
        assert settings.ocr_confidence_threshold == 0.75  # noqa: S101
        assert settings.ocr_lang == "en"  # noqa: S101

    def test_chunking_defaults(self):
        """Verify default chunking configuration."""
        settings = Settings()
        assert settings.parent_chunk_size == 1024  # noqa: S101
        assert settings.parent_chunk_overlap == 128  # noqa: S101
        assert settings.child_chunk_size == 256  # noqa: S101
        assert settings.child_chunk_overlap == 32  # noqa: S101
...
class TestSettingsProperties:
    ...
    def test_max_upload_size_bytes(self):
        ...
        settings = Settings()
        assert settings.max_upload_size_bytes == settings.max_upload_size_mb * 1024 * 1024  # noqa: S101
    ...
    def test_max_upload_size_bytes_custom(self):
        ...
        assert settings.max_upload_size_bytes == 50 * 1024 * 1024  # noqa: S101
...
class TestSettingsOverrides:
    ...
    def test_can_override_db_url(self, monkeypatch):
        ...
        settings = Settings()
        assert settings.db_url == "postgresql://user:pass@host:5432/db"  # noqa: S101
