...
class TestComputeContentHash:
    ...
    def test_computes_correct_hash(self, sample_content):
        ...
        assert compute_content_hash(sample_content) == expected  # noqa: S101
    ...
    def test_empty_content(self):
        ...
        assert compute_content_hash(b"") == expected  # noqa: S101

    def test_unicode_content(self):
        """Unicode content should hash correctly."""
        content = "こんにちは世界".encode("utf-8")
        expected = hashlib.sha256(content).hexdigest()
        assert compute_content_hash(content) == expected  # noqa: S101
...
class TestComputeTextHash:
    ...
    def test_computes_correct_hash(self, sample_text):
        ...
        assert compute_text_hash(sample_text) == expected  # noqa: S101
    ...
    def test_empty_text(self):
        ...
        expected = hashlib.sha256(b"").hexdigest()
        assert compute_text_hash("") == expected  # noqa: S101

    def test_unicode_text(self):
        """Unicode text should hash correctly."""
        text = "こんにちは世界"
        expected = hashlib.sha256(text.encode("utf-8")).hexdigest()
        assert compute_text_hash(text) == expected  # noqa: S101
...
class TestComputeFileHash:
    ...
    def test_computes_correct_hash(self, tmp_path):
        ...
        expected = hashlib.sha256(content).hexdigest()
        assert compute_file_hash(str(test_file)) == expected  # noqa: S101
    ...
    def test_pathlib_path(self, tmp_path):
        ...
        assert len(result) == 64  # SHA-256 hex length  # noqa: S101
