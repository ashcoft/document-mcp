...
import io
import logging
import subprocess  # nosec: B404 - subprocess used only to call ODA File Converter from a fixed allowlist of known install paths, no shell=True, 5min timeout
import tempfile
...
class CADExtractor:
    ...
    def _convert_dwg_to_dxf(self, dwg_path: Path, output_dir: Path) -> Path | None:
        ...
        try:
            ...
            # Only allow processing in the application directory for security
            allowed_roots = [
